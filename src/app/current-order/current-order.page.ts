...
export class CurrentOrderPage implements OnInit {
  ...
  cancelAlert() {
    this.alertTitle = 'INFORMACIÓN';
    this.alertText = '¿Estás seguro que quieres cancelar el pedido?';
    this.alertImg = '/assets/img/stop.svg';
  ...
  showAlertSuccess() {
    ...
    this.alertText = 'Te pedido fue cancelado exitosamente. Puedes seguir comprando ahora.';
  ...
  private applyStyle(): void {
    const contentElement =
      this.el.nativeElement.querySelector('.contentHeigth');
  ...
  async getCurrentOrder() {
    ...
    this.requestUseCase.getCurrentOrder(token).subscribe(
      (response) => {
        if (response.success === true) {
  ...
  async cancelCurrentOrder() {
    const token = await this.getToken();
    this.requestUseCase.cancelCurrentOrder(token).subscribe(
      (response) => {
        if (response.success === true) {
